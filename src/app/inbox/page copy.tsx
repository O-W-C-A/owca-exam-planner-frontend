...
import useDataStore, { Faculty } from '../store/courses-store';
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.map((cardData, index) => CourseCard(index, cardData))}
      </div>
    </div>
  );
}

function CourseCard(index: number, cardData: Faculty) {
  ...
  return (
    <Card key={index} className="shadow-lg">
      <CardHeader>
        <CardTitle>{cardData.longName}</CardTitle>
        <CardDescription>{cardData.shortName}</CardDescription>
      </CardHeader>
